...
Run from anywhere in the repository: python3 -B scripts/audit_lean.py
Git supplies tracked and nonignored untracked files; .lake dependencies are
excluded explicitly too. This is a lexical source guard, not a kernel audit.
Nested comments, ordinary/raw strings, character literals and escaped identifiers
are skipped. Interpolated strings are conservatively scanned (including their
embedded Lean expressions), so forbidden words in their literal text also fail.
"""
...
def violations(source):
    """Yield (one-based line, forbidden token); fail closed on unclosed comments."""
    i = 0
    while i < len(source):
